...
function getCache(key) {
  ...
  if (!item) return null
  if (Date.now() - item.ts > 30_000) { _cache.delete(key); return null }
  return item.data
}
...
router.get('/buraco-vendas/sequencia', async (req, res, next) => {
  const cacheKey = getCacheKey('seq', req.query)
...
router.get('/buraco-vendas/estoque-faturamento', async (req, res, next) => {
  try {
    const cacheKey = getCacheKey('estoque-faturamento', req.query)
    const cached = getCache(cacheKey)
    if (cached) return res.json(cached)
...
router.get('/buraco-vendas/materiais-comprados', async (req, res, next) => {
  try {
    const cacheKey = getCacheKey('materiais-comprados', req.query)
    const cached = getCache(cacheKey)
    if (cached) return res.json(cached)
