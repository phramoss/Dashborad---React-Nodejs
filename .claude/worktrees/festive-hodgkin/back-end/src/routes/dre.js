...
function toFirebirdDate(dateStr, endOfDay = false) {
  const clean = String(dateStr || '').replace(/T.*$/, '').slice(0, 10)
  return endOfDay ? clean + ' 23:59:59' : clean
}
...
router.get('/dre', async (req, res, next) => {
  ...
  try {
    ...
    if (data_ini) {
      where.push(`${campoData} >= CAST(SUBSTRING(? FROM 1 FOR 10) AS DATE)`)
      params.push(toFirebirdDate(data_ini))
    }

    if (data_fim) {
      where.push(`${campoData} <= CAST(SUBSTRING(? FROM 1 FOR 10) AS DATE)`)
      params.push(toFirebirdDate(data_fim))
    }
    ...
    const clienteExpr = `
      CASE
        WHEN VRESULTADO.NOM_PESS IS NULL OR TRIM(VRESULTADO.NOM_PESS) = ''
          THEN 'SEM CLIENTE/FORNECEDOR'
        ELSE VRESULTADO.NOM_PESS
      END
    `
