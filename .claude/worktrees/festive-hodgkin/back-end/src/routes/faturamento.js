...
router.get("/faturamento", async (req, res, next) => {
  try {
    ...
    const sql = `
     SELECT
        FAT.COD_DOC, FAT.COD_FAT, FAT.ESP_DOC, FAT.COD_EMPRESA, FAT.COD_CLIENTE,
        FAT.COD_MD, FAT.COD_MA, FAT.MATERIAL, FAT.COD_GRP, FAT.COD_VENDEDOR,
        FAT.PEDIDO, FAT.INVOICE, FAT.DATA_EMISAO, FAT.DATA_SAIDA,
        FAT.MERCADO, FAT.COBRANCA, FAT.PARCELAMENTO, FAT.CLASSIFICACAO,
        IIF(FAT.VENDEDOR = '.' OR FAT.VENDEDOR IS NULL OR FAT.VENDEDOR = '' OR FAT.VENDEDOR = ' ', 'SEM VENDEDOR', FAT.VENDEDOR) AS VENDEDOR,
        FAT.ATRAVESSADOR, FAT.VEND_ADICIONAL, FAT.TIPO,
        FAT.QTDE_PC, FAT.UNIDADE, FAT.ESP_LIQ, FAT.QTDE,
        FAT.DESCONTO, FAT.CAMBIO, FAT.TOTAL_DOCIT, FAT.TOTAL_ORIGINAL,
        FAT.NF, FAT.UF, FAT.MUN, FAT.PAIS, FAT.NOM_PESS,
