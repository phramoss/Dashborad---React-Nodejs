...
export const BVFatHierarchyMatriz = memo(function BVFatHierarchyMatriz({
  ...
  return (
    <Card noPadding className="flex flex-col overflow-hidden">
      <div className="px-3 py-2 border-b border-[var(--line)] shrink-0">
        <div className="flex items-center gap-2">
          <BarChart2 size={12} className="text-brand" />
          <p className="text-[11px] font-semibold text-text-secondary uppercase tracking-widest">Estoque por Faturamento</p>
        </div>
        ...
        <div className="overflow-x-auto overflow-y-auto" style={{ maxHeight: 293 }}>
          ...
          >
            <thead className="sticky top-0 z-10 bg-[var(--table-header-bg)]">
              <tr>
                <th rowSpan={2} style={{ width: COL_EXPAND, minWidth: COL_EXPAND, left: 0 }} className="sticky z-20 bg-[var(--table-header-bg)] border-b border-[var(--line)]" />
                <th
                  rowSpan={2}
                ...
                <th rowSpan={2} className="text-right px-2 py-1.5 text-text-muted font-medium border-b border-l border-[var(--line)] whitespace-nowrap cursor-pointer select-none hover:text-text-primary transition-colors" style={{ minWidth: COL_VAL }} onClick={() => handleSort('total')}>
              ...
              <tr>
                {periodos.map(p => (
                  <Fragment key={p}>
                    <th className="text-right px-2 py-1 text-text-muted font-medium border-b border-[var(--line)] whitespace-nowrap" style={{ minWidth: COL_VAL }}>
                      Qtde
                    </th>
                    <th className="text-right px-2 py-1 text-text-muted font-medium border-b border-r border-[var(--line)] whitespace-nowrap" style={{ minWidth: COL_VAL }}>
                      Total
                    </th>
                  </Fragment>
                ))}
            ...
            <tbody>
                ...
                return (
                  <Fragment key={item.value}>
                    <tr className={cn(
                      ...
                      <td
                        className={cn('sticky z-[5] bg-surface px-3 py-1 border-r border-[var(--line)] cursor-pointer', isSelected && 'border-l-2 border-l-brand')}
              ...
              <tr className="border-t-2 border-[var(--line)] font-semibold sticky bottom-0" style={{ background: 'var(--table-total-bg)' }}>
                <td className="sticky z-[5]" style={{ left: 0, background: 'var(--table-total-bg)' }} />
                <td className="sticky z-[5] px-3 py-1.5 text-text-secondary border-r border-[var(--line)]" style={{ left: COL_EXPAND, background: 'var(--table-total-bg)' }}>Total</td>
                {periodos.map(p => {
                  ...
                  return (
                    <Fragment key={p}>
                      ...
                      <td className="px-2 py-1.5 text-right tabular-nums text-text-primary border-r border-[var(--line)]">
                        {t ? fmtNum(t.total) : ''}
                      </td>
                    </Fragment>
                  )
