...
import { useBVTableChildren } from '@/hooks/useBuracoVendasData'
import type { BuracoVendasFiltros, EstoqueTableResult, EstoqueDrillState } from '@/types'
import {
  buildRowKey, buildChildDrill, isBVTableRowActive, applyBVTableFilter,
} from './bv-helpers'
      ...
      {rows.map(row => {
        ...
        return (
          <Fragment key={key}>
            <div
              className={cn(
                'grid grid-cols-[1fr_88px_52px] border-b border-[var(--line)]',
...
export interface BVHierarchyTableProps {
  title:    string
  icon?:    ReactNode
  headers:  string[]
  fields:   string[]
...
export const BVHierarchyTable = memo(function BVHierarchyTable({
  ...
  return (
    <Card noPadding className="flex flex-col overflow-hidden">
      ...
      <div className="grid grid-cols-[1fr_88px_52px] px-3 py-1.5 bg-[var(--table-header-bg)] border-b border-[var(--line)] shrink-0">
      ...
      {!loading && totais && (
        <div className="grid grid-cols-[1fr_88px_52px] px-3 py-1.5 bg-[var(--table-total-bg)] border-t border-[var(--line)] shrink-0">
