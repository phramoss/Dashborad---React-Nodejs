...
export const BVMateriaisComprados = memo(function BVMateriaisComprados({
  ...
  return (
    <Card noPadding className="flex flex-col overflow-hidden">
      ...
      <div
        className="grid px-3 py-1.5 border-b border-[var(--line)] shrink-0" style={{ background: 'var(--table-header-bg)' }}
        ...
        {loading ? (
          ...
          rows.map((row, idx) => (
            <div
              key={row.materialId}
              className={cn(
                'grid border-b border-r border-[var(--line)] hover:bg-surface-light/40 transition-colors',
      ...
      {!loading && rows.length > 0 && (
        <div
          className="grid px-3 py-1.5 border-t border-[var(--line)] shrink-0" style={{ background: 'var(--table-total-bg)' }}
          style={{ gridTemplateColumns: '1fr 96px 72px 72px 52px 52px 108px' }}
        >
