...
export const MultiSelect = memo(function MultiSelect({
  ...
  // Fecha ao clicar fora — setTimeout evita que o mesmo evento que abriu o
  // dropdown dispare imediatamente o handler de fechamento (bug de race condition
  // entre o click que seta open=true e o efeito que registra o listener).
  useEffect(() => {
    if (!open) return
    ...
    const timerId = setTimeout(() => {
      ...
      removeListener = () => document.removeEventListener('mousedown', onOutside)
    }, 0)
    return () => {
  ...
  return (
    <div ref={containerRef} className="relative" onKeyDown={handleKey}>
      ...
      {/* Dropdown — portal para document.body, evita problema de transform em drawers */}
